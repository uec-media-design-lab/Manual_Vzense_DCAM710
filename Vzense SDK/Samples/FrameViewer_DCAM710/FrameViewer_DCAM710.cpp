...
static void Opencv_Depth_nearest(uint32_t slope, int height, int width, uint8_t* pData, cv::Mat& dispImg)
{
	dispImg = cv::Mat(height, width, CV_16UC1, pData);
	...
	int x_s = width*2/7+10;
	int y_s = height*2/7-10;
	int x_e = width*5/7-35;
	int y_e = height*5/7-40;
	cv::Rect rect = cv::Rect(Point(x_s, y_s), Point(x_e, y_e));

	for (int j = y_s; j < y_e;j++) {
		for (int i = x_s; i < x_e;i++) {
			Point2d pointxy(i, j);
			val = dispImg.at<ushort>(pointxy);
			if (val < min && val > 0) {
				min = val;
				min_i = i;
				min_j = j;
	...
	int nothing = 0;
	if (min == 65535) nothing = 1;

	//printf("%d\n", min);
	Point2i pointxy(min_i, min_j);
	char text[20];
	sprintf_s(text, "%d", min);
	dispImg.convertTo(dispImg, CV_8U, 255.0 / slope);
	applyColorMap(dispImg, dispImg, cv::COLORMAP_RAINBOW);
	send(dispImg, pointxy, min,rect,nothing);
	Scalar red(255, 0, 0);
	circle(dispImg, pointxy, 4, red, -1, 8, 0);
	rectangle(dispImg, rect ,red,2);
	putText(dispImg, text, pointxy, FONT_HERSHEY_DUPLEX, 2,red);
	//printf("%d\n", val);
	//send();
}
...
#endif
					Opencv_Depth_nearest(slope, depthFrame.height, depthFrame.width, depthFrame.pFrameData, imageMat);
#ifdef FPS
					if (fps_tof != 0)
					{
						char fps[20];
